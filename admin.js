...
async function cargarCategoriasAdmin() {
    ...
    tabla.innerHTML = data.map(c => {
        ...
        return `
        <div class="bg-white rounded-xl sm:rounded-2xl border border-slate-200/80 shadow-sm p-2.5 sm:p-3.5 hover:shadow-md transition-shadow ${oculta ? 'opacity-60' : ''}">
            <div class="flex items-center gap-2.5 sm:gap-3">
                <span class="relative w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center flex-shrink-0 overflow-hidden">
                    ${c.imagen_url
                ...
                <div class="flex items-center gap-0.5 sm:gap-1 flex-shrink-0">
                    ...
                    <button onclick="editarCategoria(${c.id})" title="Editar" class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.75 5.25L18.75 8.25"/></svg>
                    </button>
                    <button onclick="eliminarCategoria(${c.id})" title="Eliminar" class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center text-red-400 hover:bg-red-50 hover:text-red-600 transition-colors">
...
// Muestra u oculta una categoría en la tienda pública sin borrarla. Usa la
// columna `activa` (boolean) de la tabla `categorias`; si esa columna no
// existe todavía hay que crearla en Supabase (Table Editor > categorias >
// New column > name: activa, type: bool, default value: true).
async function toggleCategoria(id, activaActual) {
    const { error } = await supabase.from('categorias').update({ activa: !activaActual }).eq('id', id);
    if (error) { mostrarToast('No se pudo actualizar la categoría.', 'error'); console.error(error); return; }

    mostrarToast(activaActual ? 'Categoría ocultada de la tienda.' : 'Categoría visible en la tienda.', 'success');
